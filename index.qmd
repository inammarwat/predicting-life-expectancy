...
ggplot(tidy_model, aes(x = term, y = estimate)) +
  geom_col(aes(fill = estimate > 0), width = 0.6, alpha = 0.8) +
  ...
  scale_y_continuous(
    ...
    labels = function(x) sprintf("%+.1f years", x)  # Add "+/-" and unit
  ) +
  coord_flip() +
  labs(
    title = "Impact of Health and Socioeconomic Factors on Life Expectancy",
    subtitle = "Education and income raise lifespan, while HIV and adult mortality shorten it",
    x = NULL,
  ...
  theme(
    ...
    axis.text.y = element_text(face = "bold", lineheight = 1.2),
    axis.text.x = element_text(size = 12),
    panel.grid.major.y = element_blank(),
    plot.caption = element_text(color = "gray50", margin = margin(t = 10))
  ) +
  geom_text(
    aes(label = sprintf("%+.2f", estimate)),  # Fixed: Changed "%+.lf" to "%+.1f"
